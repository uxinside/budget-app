...
function catRow(o, mxs) {
  ...
  /* ⚠️ 막대의 자 — 두 번 고쳤다. 왜 지금 모양인지 둘 다 적어 둔다.

     ① 아주 옛날: 넘긴 줄은 막대 전체를 「지출액」으로 놓고 예산 지점에서
        끊기만 했다. 그래서 120% 인 줄의 보라가 83% 라 95% 쓴 줄(보라 95%)
        보다 **덜 찬 것처럼** 보였다 (폴 2026-08-06).
     ② 그래서 「보라 꽉 + 넘긴 만큼(ratio−1)을 빨갛게」로 바꿨다. 그런데
        **200% 를 넘으면 빨강이 100 에서 잘려 막대를 통째로 덮는다.**
        1333% 도 210% 도 똑같이 새빨간 막대 하나였다 (폴 2026-08-11
        「카테고리 영역에 비율 표시 잘못됐어」). 자가 200% 에서 멈춘 것이다.

     지금(1.47.0): **넘긴 줄은 막대 전체가 「쓴 돈」이다.**
       보라(보이는 부분) = 예산만큼 = 100/ratio
       빨강             = 넘긴 만큼 = 100 − 100/ratio
     121% → 빨강 17 · 1333% → 빨강 93 · 2067% → 빨강 95. **절대 꽉 안 찬다.**

     ⚠️ ①의 함정을 다시 밟지 않는 이유: 보라를 100% 로 깔고 그 «위에» 빨강을
     덮는다. 넘긴 줄은 언제나 막대가 «꽉» 차 있고, 그 안에서 빨강 몫만 늘어난다.
     95% 쓴 줄(보라 95%, 빈칸 5%)보다 언제나 더 차 보인다.
     ⚠️ 빨강 상한은 98 이다. **예산이 어디까지였는지는 늘 한 조각 남아야 한다** —
     100 이 되는 순간 「얼마나 넘었나」를 다시 못 읽는다(그게 방금 그 버그였다).
